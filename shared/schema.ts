...
export const downloads = pgTable("downloads", {
  ...
  author: text("author"),
  views: text("views"),
  status: text("status").notNull().default("pending"), // pending, downloading, completed, failed, served
  downloadProgress: integer("download_progress").default(0),
  filePath: text("file_path"),
  fileToken: varchar("file_token").unique(), // Secure token for file access
  expiresAt: timestamp("expires_at"), // When the download expires
...
export const adConfig = pgTable("ad_config", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  placement: varchar("placement").notNull(), // header, footer, sidebar, interstitial, extra-interstitial, post-download
  adCode: text("ad_code"),
  ...
  adType: varchar("ad_type").default("both"), // adsense, admob, both, or image
  isEnabled: boolean("is_enabled").default(true),
  // Image ad fields
  imageDesktopUrl: text("image_desktop_url"), // Image URL for desktop (>=1024px)
  imageTabletUrl: text("image_tablet_url"), // Image URL for tablet (768px-1023px)  
  imageMobileUrl: text("image_mobile_url"), // Image URL for mobile (<768px)
  clickUrl: text("click_url"), // URL to redirect when image is clicked
  altText: varchar("alt_text"), // Alt text for accessibility
  updatedAt: timestamp("updated_at").defaultNow(),
});
