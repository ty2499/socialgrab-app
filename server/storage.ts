...
export interface IStorage {
  ...
  setAppConfig(key: string, value: string, description?: string): Promise<AppConfig>;
  
  // Admin stats operations
  getTotalDownloads(): Promise<number>;
  getTotalUsers(): Promise<number>;
  getDownloadsToday(): Promise<number>;
  getPlatformBreakdown(): Promise<Record<string, number>>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getDownloadByToken(token: string): Promise<Download | undefined> {
    const [download] = await db.select().from(downloads).where(eq(downloads.fileToken, token));
    return download;
  }
  ...
  async setAppConfig(key: string, value: string, description?: string): Promise<AppConfig> {
    const config: InsertAppConfig = { key, value, description };
  ...
  async getTotalDownloads(): Promise<number> {
    const [result] = await db.select({ count: count() }).from(downloads);
    return result.count;
  }
  ...
  async getDownloadsToday(): Promise<number> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const [result] = await db.select({ count: count() }).from(downloads)
      .where(sql`DATE(${downloads.createdAt}) = ${today.toISOString().split('T')[0]}`);
    return result.count;
  }
  ...
  async getPlatformBreakdown(): Promise<Record<string, number>> {
    const results = await db.select({
      ...
      count: count()
    }).from(downloads).groupBy(downloads.platform);
    
    return results.reduce((acc, { platform, count }) => {
      acc[platform] = count;
      return acc;
    }, {} as Record<string, number>);
  }

  async getRecentAnalytics(limit: number = 50): Promise<Analytics[]> {
  ...
  async setAdConfig(config: InsertAdConfig): Promise<AdConfig> {
    const [updated] = await db.insert(adConfig).values(config).onConflictDoUpdate({
      ...
      set: { 
        ...
        imageDesktopUrl: config.imageDesktopUrl,
        imageTabletUrl: config.imageTabletUrl,
        imageMobileUrl: config.imageMobileUrl,
